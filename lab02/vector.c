...
vector_t *vector_new() {
    ...
    retval = (vector_t*)malloc(sizeof(vector_t));/* DONE */
    ...
    retval->size = 1;/* DONE */;
    retval->data = (int *)malloc(sizeof(int)); /* DONE */;
...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    if (loc >= original_size) {
        int *longer_data = (int *)malloc((original_size) * 4 * 2);        
        for(int i = 0; i < original_size*2; i++) {
            if (i >= original_size) {
                longer_data[i] = 0;
            } else {
                longer_data[i] = v->data[i];
                }
        }
        free(v->data);
        v->data = longer_data;
